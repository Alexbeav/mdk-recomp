...
#
# Writes: dist/mdk-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix mdk-recomp \
  --exe-name MDK_Recompiled \
  --display-name "MDK Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned MDK disc" \
